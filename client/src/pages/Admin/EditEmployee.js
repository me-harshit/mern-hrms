        ...
        const { value: formValues } = await Swal.fire({
            ...
            html: `
                <div style="text-align: left; padding: 0 10px;">
                    <div style="background: #f1f5f9; padding: 15px; border-radius: 8px; margin-bottom: 20px; font-size: 15px; color: #334155; text-align: center; border: 1px solid #e2e8f0;">
        ...
        <div className="settings-container fade-in">
                        ...
                        <div className="form-grid mt-15" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}>
                            ...
                            <div className="form-group">
                                <label className="input-label" style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <span><FontAwesomeIcon icon={faWallet} style={{ color: '#215D7B', marginRight: '6px' }} /> Employee Wallet</span>
                                    <span style={{ color: walletBalance < 0 ? '#dc2626' : '#16a34a' }}>₹{walletBalance.toLocaleString('en-IN')}</span>
                                </label>
                                {(currentUser?.role === 'ADMIN' || currentUser?.role === 'HR' || currentUser?.role === 'MANAGER') ? (
                                    <div style={{ display: 'flex', gap: '10px', marginTop: '5px' }}>
                                        <button type="button" className="gts-btn" style={{ flex: 1, background: '#f0f9ff', color: '#0284c7', border: '1px solid #0284c7', height: '43px', justifyContent: 'center' }} onClick={handleManageWallet}><FontAwesomeIcon icon={faEdit} className="btn-icon" /> Manage</button>
                                        <button type="button" className="gts-btn" style={{ flex: 1, background: '#f1f5f9', color: '#475569', border: '1px solid #cbd5e1', height: '43px', justifyContent: 'center' }} onClick={fetchLedger}><FontAwesomeIcon icon={faHistory} className="btn-icon" /> Ledger</button>
                                    </div>
                                ) : (
            ...
            {activeTab === 'leaves' && (
                <div className="fade-in">
                    <div className="control-card d-block mb-20 p-25">
                        ...
                        <div className="form-grid mt-15" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
                            ...
                            <div className="form-group" style={{ display: 'flex', alignItems: 'flex-end' }}><button className="gts-btn primary full-width" style={{ height: '43px', justifyContent: 'center' }} onClick={handleUpdateBalance}><FontAwesomeIcon icon={faSave} className="btn-icon" /> Update Balances</button></div>
                        </div>
                    </div>
            ...
            {showLedger && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.6)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
                    <div className="fade-in" style={{ background: 'white', borderRadius: '12px', padding: '25px', width: '90%', maxWidth: '800px', maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e2e8f0', paddingBottom: '15px', marginBottom: '15px' }}>
                            ...
                            <button onClick={() => setShowLedger(false)} style={{ background: '#f1f5f9', border: 'none', width: '32px', height: '32px', borderRadius: '50%', cursor: 'pointer', color: '#64748b', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><FontAwesomeIcon icon={faTimes}/></button>
                        </div>
                        ...
                        <div style={{ overflowY: 'auto', flex: 1 }}>
                            <table className="employee-table" style={{ width: '100%' }}>
                                <thead>
                                    <tr><th style={{ padding: '12px 15px', background: '#f8fafc' }}>Date</th><th style={{ padding: '12px 15px', background: '#f8fafc' }}>Description</th><th style={{ padding: '12px 15px', background: '#f8fafc' }}>Auth By</th><th style={{ padding: '12px 15px', textAlign: 'right', background: '#f8fafc' }}>Amount</th></tr>
                                </thead>
                                <tbody>
                                    {filteredTransactions.length === 0 ? (
                                        <tr><td colSpan="4" style={{ textAlign: 'center', padding: '30px', color: '#94a3b8' }}>No transactions found.</td></tr>
                                    ) : (
...
// import { 
//     faUser, faSave, faArrowLeft, faClock, faPlaneDeparture, 
//     faEdit, faWallet, faHistory, faTimes 
// } from '@fortawesome/free-solid-svg-icons';
...
//                 const checkInDate = new Date(log.checkIn || new Date());
//                 const [inH, inM] = timeInStr.split(':');
//                 checkInDate.setHours(inH, inM, 0, 0);
...
//                 <button className="gts-btn cancel-btn m-0" onClick={() => navigate(`/employee/${id}`)}>
//                     <FontAwesomeIcon icon={faArrowLeft} className="btn-icon" /> Back
//                 </button>
//                 <h1 className="page-title header-no-margin">Edit Configuration: {user.name}</h1>
//             </div>
