...
import {
    ...
    faEye, faTimes, faUndo, faEdit, faBuilding, faArrowLeft
...
const AllExpenses = () => {
    ...
    // 👇 INIT FILTERS FROM URL PARAMS (Crucial for Dashboard integration)
    const [filters, setFilters] = useState({
        fromDate: '', toDate: '', 
        expenseType: searchParams.get('expenseType') || '', 
        category: searchParams.get('category') || '', 
        projectName: searchParams.get('projectName') || '',
        vendorName: searchParams.get('vendorName') || '', // Used for cross-filtering
    ...
    useEffect(() => {
        ...
        // Vendor Cross-Filter from Dashboard
        if (filters.vendorName) {
            result = result.filter(p => p.vendorId?.name === filters.vendorName);
        }

        if (filters.hasGst === 'Yes') {
            result = result.filter(p => p.expenseDetails?.gstNumber && p.expenseDetails.gstNumber.trim() !== '');
        } else if (filters.hasGst === 'No') {
            result = result.filter(p => !p.expenseDetails?.gstNumber || p.expenseDetails.gstNumber.trim() === '');
        }
        ...
        if (searchTerm) {
            ...
            result = result.filter(p =>
                ...
                (p.expenseDetails?.gstNumber && p.expenseDetails.gstNumber.toLowerCase().includes(term)) ||
    ...
    const clearFilters = () => {
        setFilters({
            fromDate: '', toDate: '', expenseType: '', category: '', projectName: '', vendorName: '',
            submittedBy: '', approvedBy: '', minAmount: '', maxAmount: '', status: '', hasGst: ''
        });
        setSearchTerm('');
    ...
    const viewFile = (fileData, title) => {
        if (Array.isArray(fileData)) {
            ...
            fileData.forEach((url, index) => {
                ...
                let mediaElement = isVideo ? `<video src="${fullUrl}" controls style="width:100%; border-radius:6px; max-height:400px; background:#000;"></video>` 
                                         : isPdf ? `<iframe src="${fullUrl}" width="100%" height="400px" style="border: none; border-radius: 6px;"></iframe>`
                                         : `<img src="${fullUrl}" style="width:100%; border-radius:6px; max-height:400px; object-fit:contain;" />`;

                htmlContent += `<div style="background: #f8fafc; padding: 10px; border-radius: 8px; border: 1px solid #e2e8f0;">
                                        ...
                                        {item.status === 'Pending' ? (
                                            <div className="flex-col gap-5">
                                                <div style={{ display: 'flex', gap: '5px' }}>
                                                    ...
                                                    <button className="gts-btn btn-small m-0" style={{ flex: 1, background: '#fee2e2', color: '#dc2626', justifyContent: 'center', padding: '6px' }} onClick={() => handleStatusUpdate(item._id, 'Rejected')}>
                                                        <FontAwesomeIcon icon={faTimesCircle} /> Reject
                                                    </button>
                                                </div>
                                                <div style={{ display: 'flex', gap: '5px' }}>
                                                    <button className="gts-btn btn-small m-0" style={{ flex: 1, background: '#fef3c7', color: '#d97706', justifyContent: 'center', padding: '6px' }} onClick={() => handleStatusUpdate(item._id, 'Returned')}>
