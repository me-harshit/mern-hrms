...
// fromDate/toDate ride along in the same object so they reach /tasks/managed
// with the rest and stack with them server-side. Empty means no date limit, so
// this list still opens on everything — unlike My Tasks, which opens on today.
const DEFAULT_FILTERS = {
    status: 'All', priority: 'All', projectId: 'All', taskType: 'All',
    fromDate: '', toDate: ''
};
...
const Tasks = () => {
        ...
        try {
            const res = await api.get('/tasks/managed', {
                params: {
                    ...
                    // The only date mode this list offers. With both ends
                    // empty the server applies no date limit at all.
                    filterType: 'Custom',
                    ...filters
                }
            });
    ...
    // From and To are two halves of one date range, so they count once
    // between them rather than twice.
    const activeFilterCount =
        Object.keys(DEFAULT_FILTERS)
            .filter(k => !['fromDate', 'toDate'].includes(k) && filters[k] !== 'All').length +
            ...
            <div className="task-toolbar">
                ...
                <div className="task-toolbar-filters">
                    ...
                    {/* Either end may be left empty for an open-ended range —
                        "everything since the 1st" is as useful as a closed one. */}
                    <div className="task-filter-dates">
                        <label className="task-filter-date">
                            <span>From</span>
                            <input
                                type="date"
                                name="fromDate"
                                className={filters.fromDate ? 'is-active' : ''}
                        ...
                        <label className="task-filter-date">
                            ...
                            <input
                                type="date"
                                name="toDate"
                                className={filters.toDate ? 'is-active' : ''}
