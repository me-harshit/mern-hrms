...
const Inventory = () => {
    ...
    useEffect(() => {
        ...
        if (searchTerm) {
            ...
            result = result.filter(item =>
                ...
                item.storageLocation?.toLowerCase().includes(term) // 👇 Updated to search by new location field
            );
        }
    ...
    const issueItems = inventory.filter(i => ['Damaged', 'Lost'].includes(i.status)).length;
                            ...
                            filteredInventory.map(item => (
                                <tr key={item._id}>
                                    ...
                                    <td data-label="Location / Assignment">
                                        {/* 👇 Updated to display the single Location string 👇 */}
